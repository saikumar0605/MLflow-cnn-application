...
def main(config_path):
    ...
    LAYERS = [ #params are converted to tuple in layer 1.
    ...
    # classifier.summary() {this is to check the summary of trainable layers.}
...
#model creation and saving...
    ...
    path_to_model = os.path.join(
        path_to_model_dir, 
        config["data"]["init_modal_file"]
        )
    
    classifier.save(path_to_model)
    logging.info(f"base model saved at {path_to_model}")

if __name__ == '__main__':
    args = argparse.ArgumentParser()
    args.add_argument("--config", "-c", default="configs/config.yaml")
    args.add_argument("--params", "-p")
